...
4. **`task check` matches dependencies by full URL prefix.** Switching a Configuration to `xpkg.crossplane.io` while the cluster has `xpkg.upbound.io` installed will produce a false-positive "missing" — same bytes, different mirror. Heads-up, not a bug.
