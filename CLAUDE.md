...
- **Crossplane CLI: the `beta` subcommands were promoted and renamed.** `beta trace` → `resource trace`, `beta validate` → `resource validate`, and `render` is now `composition render` (the bare `render` still resolves, `beta trace` does **not**). This repo's docs and `Taskfile.yaml` use the new spelling and therefore need CLI **>= v2.3.0**; `sthings.baseos` ships v2.4.1. Two things that bite: the CLI moved to its own repo at v2.3.0, so it is served from `cli.crossplane.io` as `crossplane` (not `releases.crossplane.io` as `crank`); and `composition render` runs `crossplane internal render` inside `xpkg.crossplane.io/crossplane/crossplane:stable`, a floating tag that lags and breaks render without warning — pin it with `--crossplane-image`. See [#349](https://github.com/stuttgart-things/crossplane-configurations/issues/349).
  ...
  **The floor, the CI pin and the core image live in one file: [`crossplane-versions.env`](crossplane-versions.env).** `tests/render/*.sh`, `.github/workflows/render-golden.yaml` and the `render` task all source it — don't reintroduce a second copy of any of the three numbers. `tests/render/crossplane-version.sh` guards on it: below `CROSSPLANE_MIN_VERSION` both golden scripts refuse, because an old CLI does not fail, it renders **without** `spec.crossplane.resourceRefs` and a regeneration then deletes that block from all 106 snapshots as a diff that reads like a cleanup ([#393](https://github.com/stuttgart-things/crossplane-configurations/issues/393)).
...
7. **`task push` packages the WORKING TREE, not a git ref — so a stale checkout publishes a version number over old content.** Hit on 2026-09-16: `cni` v0.1.1 was pushed from a shell sitting on a week-old branch, so the package carried the *old* composition (`xplane-cni?tag=0.1.0`) under a version annotation that advertised the fix. Nothing downstream says so: the Configuration installs, reports `HEALTHY=True`, and only the `CompositionRevision` on the cluster shows which module tag actually shipped —
   ...
   kubectl get compositionrevisions -l crossplane.io/composition-name=<name> \
     -o json | jq -r '.items[] | "rev=\(.spec.revision) \(.spec.pipeline[0].input.spec.source)"'
   ```
   The task now refuses a package directory that is dirty or differs from `origin/main`, with `FORCE=1` for the deliberate pre-release-from-a-branch case. Publish from `main` after the PR merged, not from the branch that wrote the change.
...
- **TechDocs (Backstage) are the READMEs, not copies.** `docs/index.md` and `docs/configurations/<category>/<name>.md` are GENERATED stubs (`lint-configurations.py --write`, checked like the diagram; a stub for a removed Configuration is an error). Each holds only `<!-- include-readme: <path> -->`; `.techdocs/hooks.py` includes the README at build time, turns links to another Configuration into links to its page, sends everything else to GitHub, and points "edit" at the README. So: write docs in the README, never in `docs/configurations/`. Build locally with `pip install mkdocs-techdocs-core && mkdocs build`.
