...
2. renders a KubeVirt `VirtualMachine` (`kubevirt.io/v1`) as a [provider-kubernetes](https://github.com/crossplane-contrib/provider-kubernetes) `Object` via [`function-go-templating`](https://github.com/crossplane-contrib/function-go-templating), wiring in the PVC and the cloud-init Secret, plus an Observe-only `Object` on the resulting `VirtualMachineInstance` whose reported IP is surfaced on `status.share.ip`.
