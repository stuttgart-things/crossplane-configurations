...
- **Shared defaults via EnvironmentConfig** — `spec.environmentConfig` (default `default`) selects a Crossplane `EnvironmentConfig` (matched by the label `resources.stuttgart-things.com/environment`) whose `data` supplies per-environment defaults for `folderPath`, `datacenter`, `datastore`, `resourcePool`, `network`, `template`, `annotation`, `unverifiedSsl`, `tfvarsSecretName`, `tfvarsSecretKey`, `providerConfigName` and `providerConfigKind`. Anything set explicitly on the XR spec wins; if no EnvironmentConfig matches, XR spec / built-in Workspace defaults apply. See [`examples/environmentconfig.yaml`](examples/environmentconfig.yaml).
