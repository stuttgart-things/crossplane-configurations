...
- **PipelineRun rendering via KCL** — the [`kcl-tekton-pr`](https://github.com/stuttgart-things/kcl-tekton-pr) module is pinned in the Composition (`oci://ghcr.io/stuttgart-things/kcl-tekton-pr:0.15.0`) and consumes the XR spec verbatim.
- **Deliberate re-runs via `runID`** — a Tekton `PipelineRun` is one-shot, and the wrapped Object excludes `Update` from its `managementPolicies`, so editing a completed run's spec is a **silent no-op**: the XR carries the new spec, Tekton keeps running the old one, and nothing reports `Synced=False`. Bumping `spec.runID` suffixes both `pipelineRunName` and `crossplaneObjectName` (`run-k3s` → `run-k3s-2`), which is what makes provider-kubernetes create a *new* PipelineRun. Unset reproduces the previous names exactly, so upgrading never forces a re-run. It is deliberately not derived from the spec — a hash would make an innocuous edit (a collection pin, an extra var) silently re-execute a play against a live machine. For *scheduled* re-runs use [`scheduled-run`](../scheduled-run/) instead.
