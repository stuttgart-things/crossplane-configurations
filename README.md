...
|---|---|---|---|---|
| bootstrap | [argocd-cluster](bootstrap/argocd-cluster/) | v0.2.1 | Registers a cluster with **ArgoCD**, from a namespaced `ArgocdCluster` XR — the sibling of [remote-cluster](bootstrap/remote-cluster/), which does the same for Crossplane. Emits exactly two `provider-kubernetes` Objects onto the ArgoCD cluster: an `ExternalSecret` that lets ESO pull the kubeconfig out of Vault into `argocd/{clusterName}`, and the `ClusterbookCluster` whose labels the platform ApplicationSets select on — everything downstream (IP/DNS reservation, the rendered cluster Secret, the AppProject, the components on the target) follows from those two. The kubeconfig travels Vault → ESO → Secret and is never held by Crossplane, which is also why the identity behind the ClusterProviderConfig should be scoped to `clusterbookclusters` + `externalsecrets` rather than an admin credential: that would have to live in the same `kubeconfigs/` mount every registered cluster can read. `spec.profiles` is the human-facing shape of ~30 flat Clusterbook labels, and the composition emits an explicit `'false'` for everything it disables — most AppSet selectors match with `NotIn ["false"]`, where omitting a label means ON, so a forgotten entry silently installs software. Complements [cluster](bootstrap/cluster/) with `platformEnabled: false`: a cluster gets its platform from Flux (`Platform`) or from ArgoCD (this), never both. Teardown ordering is load-bearing — the AppSets carry `preserveResourcesOnDeletion: true`, so deregistering first leaves orphaned Applications behind that have no `ownerReference` to collect them. | [`ghcr.io/stuttgart-things/crossplane-configurations/argocd-cluster`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fargocd-cluster) |
| bootstrap | [capability](bootstrap/capability/) | v0.7.2 | Configures what a management cluster can **do**. A provider is installed on every management cluster and on its own does nothing useful: without a `ClusterProviderConfig` it does not know where to connect, without credentials it may not log in, and without an `EnvironmentConfig` it does not know which node, datastore or template to place a VM on. A namespaced `Capability` XR names capabilities (`proxmoxvm`, `vspherevm`, `ansible-run`) and their placement facts; the Composition emits the `ClusterSecretStore`, the credential `ExternalSecret`, the `ClusterProviderConfig` and the `EnvironmentConfig` per capability onto a target cluster — rendered by `function-kcl` (KCL module [`xplane-capability`](https://github.com/stuttgart-things/kcl/tree/main/crossplane/xplane-capability), structure from [`xplane-capability-catalog`](https://github.com/stuttgart-things/kcl/tree/main/crossplane/xplane-capability-catalog)). The native replacement for the per-capability Helm charts on clusters with external-secrets: the two values those charts must be handed (`authMountPath`, the store name) are derived here from the cluster the XR already names — which only means something because the store is emitted here too, one per capability, since a `ClusterSecretStore` fixes one Vault KV mount and capabilities do not share one, and package installation stays with [management-plane](bootstrap/management-plane/) rather than being a second source of lock nodes. `spec.workloadNamespace` is separate from `spec.namespace` because a capability's cloud-init password must sit where the VM XRs are, not beside the provider. The placement keys are the catalog's, and a Composition-side key the catalog does not carry is silence rather than an error — `proxmoxvm`'s `cloudInitDatastore` was missing until catalog 0.5.0, so every capability-built EnvironmentConfig let the cloud-init drive fall through to the root disk's datastore, which in LabUL is the storage whose stop/start round trip leaves the VM unbootable. Every namespace it writes into is emitted as a `Namespace` with `managementPolicies: [Observe, Create]` rather than assumed: which one it is comes out of the capability's own placement — `ansible-run` lands in `tekton-ci` because a Tekton pipeline reads it there — so on a cluster whose platform does not happen to install the app that brings that namespace along, the credentials Object simply sits at `namespaces "tekton-ci" not found` while the rest of the XR reports green. A missing or unknown placement field fails the render naming every problem at once, since a missing `node` otherwise surfaces minutes later as a VM placed nowhere. Readiness differs per emitted kind: the `ClusterSecretStore` and the `ExternalSecret`s derive it from their own conditions, because those are the only evidence Vault answered, while the status-less `EnvironmentConfig` and `ClusterProviderConfig` would otherwise sit at `Ready=False` forever (measured, [#294](https://github.com/stuttgart-things/crossplane-configurations/issues/294)). | [`ghcr.io/stuttgart-things/crossplane-configurations/capability`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fcapability) |
...
| bootstrap | [cluster](bootstrap/cluster/) | v0.11.10 | Builds a whole Kubernetes cluster from one namespaced `ClusterStack` XR: a VM ([proxmoxvm](machinery/proxmoxvm/) / [vspherevm](machinery/vspherevm/), with base-OS ansible), the distribution install and the kubeconfig upload ([ansible-run](cicd/ansible-run/)), a `ClusterAccess` ([remote-cluster](bootstrap/remote-cluster/)) and a `Platform` ([platform](bootstrap/platform/)) — all as child XRs, rendered by `function-kcl` (KCL module [`xplane-cluster`](https://github.com/stuttgart-things/kcl/tree/main/crossplane/xplane-cluster), profiles from [`xplane-cluster-catalog`](https://github.com/stuttgart-things/kcl/tree/main/crossplane/xplane-cluster-catalog)). Replaces the five hand-written XRs a cluster needs today, in which the cluster name appears 8× and the VM IP 3× — both derived here instead. Each stage is gated on the previous having SUCCEEDED (not merely Ready) and every gate is sticky, so a guest-agent blip cannot delete an `AnsibleRun` and re-run its play against a live machine; three `Usage` resources order teardown. `platform.cni.enabled` follows from the distribution's CNI ownership rather than being restated. Distributions: k3s, kind, single-node rke2 — and the rancher pair `rancher-k3s` / `rancher-rke2`, where Rancher creates the cluster and the stack composes a `RancherCluster` first, the VM waits for the node-registration command it publishes, and one join stage replaces the distribution + kubeconfig pair. The two rancher entries are not interchangeable: `machineGlobalConfig` keys belong to the server Rancher provisions, k3s and rke2 do not share them, and an unknown key is ignored rather than refused, so the wrong one leaves the bundled CNI and ingress running and the cluster never registers ([kcl#282](https://github.com/stuttgart-things/kcl/issues/282)). In rancher mode an app profile also brings its secrets: the stack reads the [app-secret-profile](vault/app-secret-profile/) declarations the catalog names and composes a [vault-secrets](vault/vault-secrets/) `VaultSecretSet` per app, derives the stores and the platform's secrets gate, and fails the render rather than compose less. | [`ghcr.io/stuttgart-things/crossplane-configurations/cluster`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fcluster) |
| bootstrap | [cni](bootstrap/cni/) | v0.1.4 | Installs a CNI on a target cluster from a namespaced `Cni` XR — [cilium](https://cilium.io/) today (the provider is an enum, so more can follow). Emits a `provider-helm` Release for the chart plus a `provider-kubernetes` Object that observes the target's `RemoteCluster` to gate the install until the derived `{clusterName}-helm` ClusterProviderConfig exists — rendered by `function-kcl` (KCL module [`xplane-cni`](https://github.com/stuttgart-things/kcl/tree/main/crossplane/xplane-cni)). Settings that are cluster-shape facts rather than free-form Helm values (`kubeProxyReplacement`, `k8sServiceHost`) are named fields with defaults derived from `clusterName`, because getting them wrong deadlocks the cluster silently; `spec.values` stays open for the rest. The gate Object carries a readiness query of its own, so neither `status.ready` nor the XR's Ready condition can go true ahead of the Release ([#439](https://github.com/stuttgart-things/crossplane-configurations/issues/439)) — [platform](bootstrap/platform/) gates its other components on that. | [`ghcr.io/stuttgart-things/crossplane-configurations/cni`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fcni) |
...
| k8s | [cloud-config](k8s/cloud-config/) | v0.5.5 | Renders cloud-init userdata as a Kubernetes Secret from a namespaced `CloudInit` XR, and manages the target namespace. | [`ghcr.io/stuttgart-things/crossplane-configurations/cloud-config`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fcloud-config) |
...
| machinery | [harvester-vm](machinery/harvester-vm/) | v0.1.12 | Provisions a Harvester / KubeVirt virtual machine from a namespaced `HarvesterVM` XR. Composes the `volume-claim` (root disk PVC) and `cloud-config` (cloud-init Secret) Configurations, then renders a KubeVirt `VirtualMachine` via `provider-kubernetes`; shared placement/image default from an `EnvironmentConfig`. | [`ghcr.io/stuttgart-things/crossplane-configurations/harvester-vm`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fharvester-vm) |
| machinery | [proxmox-vm](machinery/proxmox-vm/) | v0.2.0 | Provisions a Proxmox VE virtual machine from a namespaced `ProxmoxVM` XR. An OpenTofu `Workspace` (`provider-opentofu`) runs the [`proxmox-vm`](https://github.com/stuttgart-things/proxmox-vm) Terraform module; shared placement/credentials default from an `EnvironmentConfig`. | [`ghcr.io/stuttgart-things/crossplane-configurations/proxmox-vm`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fproxmox-vm) |
| machinery | [proxmoxvm](machinery/proxmoxvm/) | v0.16.0 | Provisions a Proxmox VE virtual machine from a namespaced `NativeProxmoxVM` XR using the NATIVE [`provider-proxmox-bpg`](https://github.com/valkiriaaquatica/provider-proxmox-bpg) managed `EnvironmentVM` resource (no Terraform/OpenTofu; built on `bpg/terraform-provider-proxmox`). The native-provider sibling of `proxmox-vm`: clones by template VMID and bootstraps via cloud-init (`initialization`); shared placement defaults from an `EnvironmentConfig`, rendered by `function-kcl`. Optional base-OS provisioning emits an `AnsibleRun` once the VM is Ready with an IP; it forwards only the fields the XR actually sets and leaves the AnsibleRun's own `environmentConfig` alone, so ansible-run resolves against `ansible-run-defaults` ([#317](https://github.com/stuttgart-things/crossplane-configurations/issues/317) passed `spec.environmentConfig` down instead; that names a PROXMOX environment, matched no ansible-run `EnvironmentConfig`, and silently dropped every default including `ansibleExtraCollections`). Set `ansible.environmentConfig` explicitly if a per-environment ansible-run `EnvironmentConfig` exists. The cloud-init drive takes its own datastore (EnvironmentConfig `cloudInitDatastore`) rather than following the data disk: it is the one disk PVE frees and re-allocates on every stop/start, and on LabUL's `V5010-01-1` that leaves an orphaned LV behind which makes the VM unbootable until the volume is deleted through the API. Tags (`vm.tags`, EnvironmentConfig `tags`) are MERGED across the two rather than overridden — a VM that names one tag of its own would otherwise lose its environment's — then sorted and deduped, because Proxmox returns a tag set alphabetically and an unsorted list reads as a difference on every reconcile. Emitting them at all makes the emitted list the VM's WHOLE tag set: `tags` is absent from `status.atProvider`, so the provider cannot merge with the tags a full clone inherited from its template, nor with `plb_*` scheduler tags set by hand ([#484](https://github.com/stuttgart-things/crossplane-configurations/issues/484)). | [`ghcr.io/stuttgart-things/crossplane-configurations/proxmoxvm`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fproxmoxvm) |
| machinery | [rancher-cluster](machinery/rancher-cluster/) | v0.10.0 | Provisions a generic (custom-node) Rancher cluster from a namespaced `RancherCluster` XR (k3s/rke2 `provisioning.cattle.io` Cluster), then wires the downstream cluster's Rancher-managed kubeconfig Secret into a `provider-kubernetes` `ClusterProviderConfig` and bootstraps a namespace on it as proof — all via `function-go-templating`. Optionally registers the cluster in Argo CD via [clusterbook-operator](https://github.com/stuttgart-things/clusterbook-operator) (assembles a proxy kubeconfig with a dedicated non-expiring token), and optionally reserves a clusterbook IP/DNS (`spec.argocd.reservation`) to stamp the `allocation-ip` anchor the network-platform ApplicationSets require. When `network-platform/cert-manager-vault-pki` is enabled it also inlines the [vault-pki-secrets](bootstrap/vault-pki-secrets/) logic — pushing the Vault token + CA Secrets onto the downstream cluster's `cert-manager` namespace so the `vault-pki` ClusterIssuer has its prerequisites. | [`ghcr.io/stuttgart-things/crossplane-configurations/rancher-cluster`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Francher-cluster) |
| machinery | [virtual-machine](machinery/virtual-machine/) | v0.1.14 | High-level VM API: a namespaced `XVirtualMachine` XR asks for a VM by t-shirt `size`, `provider` (vsphere/proxmox) and `environment`, resolves the datacenter topology from an `EnvironmentConfig`, and emits a `VMProvision` (via `function-kcl`). | [`ghcr.io/stuttgart-things/crossplane-configurations/virtual-machine`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvirtual-machine) |
| machinery | [vm-batch](machinery/vm-batch/) | v0.1.3 | Batch VM provisioning (+ Ansible): a namespaced `VMBatch` XR fans out a list of VM definitions — each with an optional `count` of identical replicas — to the native `proxmoxvm` / `vspherevm` Configurations (per `spec.provider`), then optionally emits a SINGLE `AnsibleRun` whose inventory lists every VM IP so one play configures the whole fleet (via `function-kcl`). | [`ghcr.io/stuttgart-things/crossplane-configurations/vm-batch`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvm-batch) |
...
| machinery | [vspherevm](machinery/vspherevm/) | v0.12.0 | Provisions a VMware vSphere virtual machine from a namespaced `NativeVsphereVM` XR using the NATIVE [`provider-vspherevm`](https://github.com/stuttgart-things/provider-vspherevm) managed `VirtualMachine` resource (no Terraform/OpenTofu). The native-provider sibling of `vsphere-vm`: per-environment placement MOIDs (`templateUuid`, `datastoreId`, `resourcePoolId`, `networkId`, `folder`) are loaded from a Crossplane `EnvironmentConfig`, so an XR states only intent (name/cpu/ram/disk) and the topology is injected. `vm.tags` and `vm.customAttributes` mirror proxmoxvm's, with the difference that decides how they are used: a vSphere tag is an inventory OBJECT, so the field takes its id (`urn:vmomi:InventoryServiceTag:<uuid>:GLOBAL`) and a custom attribute its numeric vCenter field id — and since `provider-vspherevm` ships only `VirtualMachine`, nothing here can create or resolve either, so an unknown id fails the clone. Tags merge with the EnvironmentConfig's and are sorted and deduped (the CRD declares the field a set, which rejects a repeat outright); custom attributes merge per key with the XR winning ([#484](https://github.com/stuttgart-things/crossplane-configurations/issues/484)). | [`ghcr.io/stuttgart-things/crossplane-configurations/vspherevm`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvspherevm) |
| storage | [minio](storage/minio/) | v0.1.0 | MinIO **buckets, policies and users** from a cluster-scoped `MinioBucket` XR, composed as native [provider-minio](https://github.com/vshn/provider-minio) managed resources — the bucket, one `Policy` per `spec.access` entry (`<bucket>-<entry>`, either scoped to this bucket or a raw S3 policy) and, unless `user: false`, one `User` carrying it whose keys land in a Secret as `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` — the shape an S3 backend, CNPG's barman plugin and rclone all read. Replaces the `mc` commands every bucket in the fleet was created with. `spec.deletionPolicy` decides what deleting the XR does to the BUCKET (`Orphan` by default, because a bucket holds data this Configuration did not write; `DeleteIfEmpty`, `DeleteAll`), while policies and users always go — they are access, not data. Cluster-scoped on purpose: every kind provider-minio serves is cluster-scoped and Crossplane v2 refuses to compose one from a namespaced XR, and a bucket name is unique per instance rather than per namespace anyway (reason recorded on the XRD). Two provider behaviours shape the Composition, both measured on a lab MinIO with v0.4.5: a `Policy` is named after its Kubernetes OBJECT and ignores `crossplane.io/external-name`, so a generated resource name would reach the instance and the user referencing it is refused with `policy not found` — every composed object therefore carries an explicit `metadata.name`; and an existing bucket cannot be adopted (`observe failed: bucket already exists`), so the hand-made buckets in the fleet stay outside Crossplane until that changes. A `User` is composed only once its `Policy` is Ready, because provider-minio validates the policy's existence in an admission webhook. | [`ghcr.io/stuttgart-things/crossplane-configurations/minio`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fminio) |
| vault | [app-secret-profile](vault/app-secret-profile/) | v0.1.1 | What an app needs in Vault, declared once per app as a **cluster-scoped** `AppSecretProfile` (group `secrets.stuttgart-things.com`, name = the app, as listed by a catalog profile's `appSecrets`): entries the app owns per cluster, each key exactly one of `generate` / `literal` / `shared` (enforced at admission), which keys an order may override, and `reads` of other apps' keys used directly, never copied. Composes nothing — publishes the resolved view and checks every read reference (`ReadsResolved`). Consumed by `xplane-cluster`; the environment half stays in `cluster-vault-<env>` (#464). Cluster-scoped on purpose: a profile decides what every order of that app receives, so only admins or the app owners may change it. | [`ghcr.io/stuttgart-things/crossplane-configurations/app-secret-profile`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fapp-secret-profile) |
| vault | [vault-k8s-auth](vault/vault-k8s-auth/) | v0.4.2 | **Prototype.** Vault Kubernetes auth backends from a namespaced `VaultK8sAuth` XR (group `vault.stuttgart-things.com`) as native [provider-vault](https://github.com/upbound/provider-vault) managed resources — auth mount, created policies, role and TokenReview config each their own MR — instead of one OpenTofu `Workspace` per auth like [vault-auth](bootstrap/vault-auth/). The role is composed only once the mount and every policy it names are Ready (Vault accepts a role naming a missing policy and the token then silently has no permissions); `status.share` keeps vault-auth's contract. `adoption` takes over what vault-auth already created for a cluster: Observe-only first, compared with the spec, handed over without Delete — differences hold it and are named in a condition. Created policies carry the fixed prefix `xp-`, so the AppRole needs only `sys/policies/acl/xp-*`; `policies[].kvMount` is denied for admin-credential mounts (`kubeconfigs`, `ssh`, `cicd-*`) and Vault system paths at admission; everything else, including `tokenPolicies`, is bounded by RBAC on the XR. | [`ghcr.io/stuttgart-things/crossplane-configurations/vault-k8s-auth`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvault-k8s-auth) |
| vault | [vault-secrets](vault/vault-secrets/) | v0.4.0 | **Prototype.** Writes secrets into a Vault KV engine (v2 or v1) from a namespaced `VaultSecretSet` XR via provider-vault. Each key is a literal, a key of a Secret in the XR's own namespace, generated once at random, or adopted once from an existing Vault entry (`vaultRef`, through the XR's own credential) — kept and read back on every reconcile; the JSON sits in a composed Secret next to the XR and reaches Vault by `dataJsonSecretRef`, never in an MR spec. Losing that Secret after Vault was written holds the XR at `Ready=False` instead of silently rotating a live credential, unless `generate.regenerate: true`. A secret is written only once every source resolves; a source that disappears later keeps the last value and flags `SourcesResolved=False`. A `claim` owns an entry without its data — custom_metadata only, and every version deleted with the XR — for paths another writer fills, like `kubeconfigs/<cluster>`. Can compose the KV mount, which it never deletes. | [`ghcr.io/stuttgart-things/crossplane-configurations/vault-secrets`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvault-secrets) |
...
└── vault/                        # Vault objects as native provider-vault MRs (prototype)
    ├── app-secret-profile/       # per-app secret declarations (cluster-scoped), read by xplane-cluster
    ├── vault-k8s-auth/           # Kubernetes auth backends, policies, roles
    └── vault-secrets/            # KV secrets from Secret refs, literals or generated values
```
