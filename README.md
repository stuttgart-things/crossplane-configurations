...
| vault | [vault-k8s-auth](vault/vault-k8s-auth/) | — | **Prototype.** Vault Kubernetes auth backends from a namespaced `VaultK8sAuth` XR (group `vault.stuttgart-things.com`) as native [provider-vault](https://github.com/upbound/provider-vault) managed resources — auth mount, created policies, role and TokenReview config each their own MR — instead of one OpenTofu `Workspace` per auth like [vault-auth](bootstrap/vault-auth/). The role is composed only once the mount and every policy it names are Ready (Vault accepts a role naming a missing policy and the token then silently has no permissions); `status.share` keeps vault-auth's contract. | *not published* |
| vault | [vault-secrets](vault/vault-secrets/) | — | **Prototype.** Writes secrets into a Vault KV engine (v2 or v1) from a namespaced `VaultSecretSet` XR via provider-vault. Each key is a literal, a key of a Secret in the XR's own namespace, or generated once at random and read back on every reconcile; the JSON sits in a composed Secret next to the XR and reaches Vault by `dataJsonSecretRef`, never in an MR spec. A secret is written only once every source resolves; a source that disappears later keeps the last value and flags `SourcesResolved=False`. Can compose the KV mount, which it never deletes. | *not published* |
