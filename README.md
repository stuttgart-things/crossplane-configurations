...
| vault | [vault-secrets](vault/vault-secrets/) | v0.3.0 | **Prototype.** Writes secrets into a Vault KV engine (v2 or v1) from a namespaced `VaultSecretSet` XR via provider-vault. Each key is a literal, a key of a Secret in the XR's own namespace, or generated once at random and read back on every reconcile; the JSON sits in a composed Secret next to the XR and reaches Vault by `dataJsonSecretRef`, never in an MR spec. Losing that Secret after Vault was written holds the XR at `Ready=False` instead of silently rotating a live credential, unless `generate.regenerate: true`. A secret is written only once every source resolves; a source that disappears later keeps the last value and flags `SourcesResolved=False`. A `claim` owns an entry without its data — custom_metadata only, and every version deleted with the XR — for paths another writer fills, like `kubeconfigs/<cluster>`. Can compose the KV mount, which it never deletes. | [`ghcr.io/stuttgart-things/crossplane-configurations/vault-secrets`](https://github.com/stuttgart-things/crossplane-configurations/pkgs/container/crossplane-configurations%2Fvault-secrets) |
