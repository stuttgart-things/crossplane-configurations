...
3. **RBAC for the in-cluster provider-kubernetes SA** — see [`examples/rbac.yaml`](examples/rbac.yaml). Two rules, both found by a wedged live build:
   - **create RemoteClusters.** Easy to miss, because it was never needed before: RemoteClusters have always been applied by hand, so the provider only ever had to *read* existing ones. The first Composition to create one fails with `cannot create resource "remoteclusters" … at the cluster scope`.
   - **patch `clusterproviderconfigs.helm.m.crossplane.io`** — needed even though the Object that touches them is `managementPolicies: ["Observe"]`. provider-kubernetes computes desired state with a **dry-run server-side apply**, and the API server authorizes a dry-run SSA exactly like a real write. The symptom is misleading: `status.ready` goes **true** and `status.share` carries the endpoint and `serverVersion` (both come from the RemoteCluster, which is fine), while only Crossplane's own `Ready` condition stays False with `Unready resources: observe-helm`. Drop this rule if `spec.providers` omits `helm`.

   Bind the **group** `system:serviceaccounts:crossplane-system`, not the ServiceAccount: the real SA name carries a package-revision suffix (`crossplane-contrib-provider-kubernetes-f6665ef36536`) that changes on every provider upgrade, so a name-bound ClusterRoleBinding goes stale silently. Not needed at all where the provider's SA is already cluster-admin — check with `kubectl auth can-i`, see the file header.
