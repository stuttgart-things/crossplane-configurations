...
Catalogued as of `xplane-platform` 0.21.0 (catalog 0.17.0). `stuttgart-things/flux` publishes `apps/`, `infra/` and — since [flux#196](https://github.com/stuttgart-things/flux/pull/196) widened the release workflow's `find` — `cicd/`, which is what made `tekton` catalogable at all; the rest of `cicd/` (argo-cd, crossplane, komoplane, kro, …) is publishable now but not yet catalogued. These are the ones a `Platform` can name:
